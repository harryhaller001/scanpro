...
def ebayes(
    ...
    stdev_coef_lim: List[float] = [0.1, 4.0],
    robust: bool = False,
    winsor_tail_p: List[float] = [0.05, 0.1],
) -> dict:
    """
    ...
    if (
        not coefficients.any()
        or not sigma.any()
        or not stdev.any()
        or not df_residual.any()
    ):
    ...
    var_prior, var_post, df_prior = squeeze_var(
        sigma**2, df_residual, robust=robust, winsor_tail_p=winsor_tail_p
    )
    ...
    res["t"] = (
        coefficients / stdev / np.reshape(np.sqrt(res["s2_post"]), (n_clusters, 1))
    )
    ...
    res["p_value"] = 2 * t.cdf(
        -abs(res["t"]), df=np.reshape(df_total, (len(res["t"]), 1))
    )

    # B-statistics
    var_prior_lim = np.array(stdev_coef_lim) ** 2 / res["s2_prior"]
    res["var_prior"] = tmixture_matrix(
        res["t"], stdev, df_total, proportion, var_prior_lim
    )
    ...
    if any(inf_df):
        ...
        if any(~inf_df):
            ...
            kernel[~inf_df] = (
                ...
                / 2
                * np.log(
                    (t2_f + df_total_f[:, np.newaxis])
                    / (t2_f / r_f + df_total_f[:, np.newaxis])
                )
            )
    else:
        kernel = (
...
def squeeze_var(
    var: NDArray,
    df: NDArray,
    covariate: Optional[NDArray] = None,
    robust: bool = False,
    winsor_tail_p: List[float] = [0.05, 0.1],
    ...
    :param Optional[np.ndarray] covariate: defaults to None.
    :param bool robust: Apply robust empirical bayes method, defaults to False
    :param List[float] winsor_tail_p: Limits for winsorization; this will set outliers below min/max percentile
    ...
    :raises ValueError: When not correctly estimated.
    :return Tuple[NDArray, NDArray, NDArray]: Estimated var prior, df prior and adjusted var post.
    """
    n = len(var)
    if n == 0:
    ...
    if robust:
        fit = fit_f_dist_robust(var, df, covariate, np.array(winsor_tail_p))
    ...
    if not any(df_prior) or any(np.isnan(df_prior)):
        ValueError("Could not estimate prior!")
...
def tmixture_matrix(
    ...
    for i in range(n_coef):
        v0[i] = tmixture_vector(
            t_stat[:, i], stdev_unscaled[:, i], df, proportion, v0_lim
        )
...
def classify_tests_f(
    ...
    df: pd.DataFrame = fit["df_total"] + fit["df_prior"]
    df[df == np.inf] = 1e10
    tstat = fit["t"]
